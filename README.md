...
Local-first Angular, Spring, and PostgreSQL application foundation for
`KJG Repostería y Manualidades`.

Agenda KJG is being initialized for baking and crafts management. Business
modules are intentionally not present yet; this repository currently provides
the product identity, Angular shell, authentication foundation, public health
endpoint, PostgreSQL persistence, Flyway migrations, tests, and CI validation.

End-user interface content uses Spanish for Mexico (`es-MX`). Code,
identifiers, branches, commits, prompts, tests, logs, database objects, API
contracts, and technical documentation remain in English.

Product defaults:

- Currency: `MXN`
- Business timezone: `America/Mexico_City`
- End-user locale: `es-MX`

System boundaries still use UTC instants where required by `ARCHITECTURE.md`.
The business timezone is documented for future domain behavior and does not
change the JVM, PostgreSQL, browser, or system timezone.
...
If a previous local PostgreSQL volume was initialized with old database or user
defaults, stop and request explicit approval before deleting or recreating any
Docker volume.
